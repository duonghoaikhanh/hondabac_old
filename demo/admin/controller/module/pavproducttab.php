...
class ControllerModulepavproducttab extends Controller {
	private $error = array(); 
	
	public function index() {   
		
		$this->language->load('module/pavproducttab');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			unset(  $this->request->post['action_mode']	 );
 			...
 			$this->model_setting_setting->editSetting('pavproducttab', $this->request->post);		
		...
		if (isset($this->error['dimension'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pavproducttab', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/pavproducttab', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['modules'] = array();
		
		if (isset($this->request->post['pavproducttab_module'])) {
			$this->data['modules'] = $this->request->post['pavproducttab_module'];
		} elseif ($this->config->get('pavproducttab_module')) { 
			$this->data['modules'] = $this->config->get('pavproducttab_module');
		}	
		...
		$this->data['tabs'] = $tabs;
		$this->template = 'module/pavproducttab.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/pavproducttab')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		
		if (isset($this->request->post['pavproducttab_module'])) {
			foreach ($this->request->post['pavproducttab_module'] as $key => $value) {
